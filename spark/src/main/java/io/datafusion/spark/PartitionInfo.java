 ...
 *   <li>{@code partitionBytes} — opaque per-partition payload. Bridge encodes whatever the
 *       executor needs to materialise *this* slice (offsets, row ranges, sub-options, etc.).
 *       Combined with the global {@code optionsProtoBytes} in {@link
 *       FfiProviderFactory#createProvider(byte[], byte[])}. Empty array = no per-partition state
 *       (single-partition table).
 *   <li>{@code preferredLocations} — hostnames where this partition's data lives. Returned from
 *       {@code InputPartition.preferredLocations()} so Spark can co-locate the task with the
 *       data. Empty array = no preference. Honoured subject to {@code spark.locality.wait}.
 * </ul>
 */
public record PartitionInfo(String id, byte[] partitionBytes, String[] preferredLocations) {
